...
err_t tcp_server_recv(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    ...
    if (!p) {
        ...
        tcp_server_close(arg); // Close this client connection
    ...
    // Prepare a response message in a buffer
    const char *response = "SSThis is the server"; // Response message

    // Send response
    err = tcp_write(tpcb, response, strlen(response), TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) {
        DEBUG_printf("Failed to send response: %d\n", err);
        return ERR_OK;  // Keep the connection alive
    }
    ...
    tcp_server_close(arg); // Close this client connection without stopping the server
